...
const articleCards = [
  { title: 'ESTA(エスタ)とは？', href: '/list-esta-application/esta', gradient: 'card-gradient-1' },
  { title: '公式ウェブサイトについて解説', href: '/list-esta-application/official-cbp', gradient: 'card-gradient-2' },
  { title: '出生地や住所の書き方', href: '/list-esta-application/place-of-birth', gradient: 'card-gradient-3' },
  { title: 'ESTAの申請方法を解説', href: '/list-esta-application/esta-flow', gradient: 'card-gradient-4' },
  { title: '公式ウェブサイトでの申請方法', href: '/list-esta-application/cbp-flow', gradient: 'card-gradient-5' },
  { title: '米国内の連絡先情報', href: '/us-contact-details', gradient: 'card-gradient-6' },
  { title: 'ESTAの有効期限と再申請', href: '/list-esta-application/expiration-date', gradient: 'card-gradient-1' },
  { title: 'アプリのESTA申請方法', href: '/list-esta-application/application', gradient: 'card-gradient-2' },
  { title: 'グアム入国にESTAは必要？', href: '/list-esta-application/guam', gradient: 'card-gradient-5' },
];
...
export default function Page() {
  const faqItems = [
    ...
    { q: 'ESTAを申請するタイミング・申請期限について', a: 'ESTA(エスタ)の審査には、最大3日間かかる場合があります。そのため、<a href="https://www.cbp.gov/" target="_blank" rel="noopener noreferrer" className="text-[#c41e3a] hover:underline">CBP(アメリカ合衆国税関・国境警備局)</a>や<a href="https://www.mofa.go.jp/mofaj/toko/passport/us_esta.html" target="_blank" rel="noopener noreferrer" className="text-[#c41e3a] hover:underline">外務省</a>では、出発日の72時間前までにESTAを申請し、「渡航認証許可」を取得することを推奨しています。出発日に申請しても、当日中にESTAが承認されるとは限らないため十分ご注意ください。なお、ESTAは出発日や宿泊先が未定でも申請可能です。アメリカへの渡航が決定した時点で、余裕を持って早めに申請することをお勧めします。また、家族やグループで渡米する場合は、全員分のESTAが必要です。複数人分の申請を行う際は、申請フォームから1名ずつ個別にお手続きください。同行者のうち１人でも申請内容に不備があると、飛行機への搭乗を拒否される可能性があるため、出発の3日前までに全員がESTAを取得済みかどうか必ず確認しましょう。' },
    ...
    { q: 'ESTA公式サイトについて', a: '<a href="https://www.cbp.gov/" target="_blank" rel="noopener noreferrer" className="text-[#c41e3a] hover:underline">CBP(アメリカ合衆国税関・国境警備局)</a>公式サイトからESTA(エスタ)を申請する場合は、<a href="https://esta.cbp.dhs.gov/" target="_blank" rel="noopener noreferrer" className="text-[#c41e3a] hover:underline">こちら</a>よりお手続きください。公式サイトは日本語表示に切り替えることが可能ですが、審査はCBPが行うため、申請フォームへの入力はすべて英語(ローマ字)で行う必要があります。また、問い合わせ対応も英語のみとなるためご注意ください。 US ESTA Apply Websiteでは、日本語でのお問い合わせ対応や、分かりづらい入力項目のサポートを通じて、お客様の<Link href="/list-site-information/entry" className="text-[#c41e3a] hover:underline">ESTA申請</Link>（お申し込み）を日本語でサポートいたします。' },
  ];
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {articleCards.map((card, i) => (
              <Link key={i} href={card.href} className="block rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all group">
                <div className={`${card.gradient} h-44 img-hover-shrink flex items-center justify-center`}>
